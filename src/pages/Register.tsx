...
const Register: React.FC = () => {
  ...
  const { registerUser, isAuthenticated, error, clearError } = useAuthStore();
  ...
  const requirements = [
    ...
    { id: 'match', label: 'Passwords match', met: password && password === confirmPassword }
  ];
  
  // Check if user is already authenticated
  useEffect(() => {
    if (isAuthenticated) {
      navigate('/onboarding'); // Always go to onboarding after registration
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If successful, user will be redirected to onboarding via the useEffect
    } catch (error) {
